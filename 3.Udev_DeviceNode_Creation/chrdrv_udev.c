...
static int __init char_dev_init(void)
{
  ...
  /* An entry is created in /sys/class/VIRTUAL */
  device_class = class_create(THIS_MODULE, "VIRTUAL");

  /* A file/node is created automatically in /dev/<CHAR_DEVICE_NAME> */
  /* Here the name can be different from CHAR_DEVICE_NAME, in that case the 
     device will appear under different names under /proc/devices and /dev.
     The userApp can access the driver using /dev/<Name> */
...
MODULE_DESCRIPTION("Character Driver Skeleton with Automatic node creation in /dev using hotplug subsystem");
MODULE_LICENSE("GPL");
